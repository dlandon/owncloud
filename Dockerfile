...
RUN	apt-get -y install mariadb-server mysqltuner sudo nano && \
	...
	apt-get -y install libmysqlclient18 libpcre3-dev libsmbclient.dev nginx openssl php$PHP_VERS-bz2 php$PHP_VERS-cli && \
	...
	apt-get -y install redis-server php-redis php-apcu
